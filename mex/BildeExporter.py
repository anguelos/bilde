...
class FunctionParameter:
    @staticmethod
    def extractTemplateParams(typeStr):
        if typeStr.count('<')>0:
            return [typeStr.split('<')[0]]+[extractTemplateParams for p in params.split(',')]
        else:
            return typeStr
    def __init__(self,parameterStr,templateTypes):
        self.parameterStr=parameterStr
        self.typeStr=parameterStr.split(' ')[0];
        self.nameStr=parameterStr.split(' ')[1];
        templateDepth=parameterStr.count('<')
    def isInput(self):
        return self.nameStr.find('in')==0
    def isOutput(self):
        return (self.nameStr.find('out')==0) or (self.nameStr.find('inout')==0)
    def isTemplate(self):



class BildeExporter:
    def __init__(self,functionDeclarationStr,templateArguments):
        funcDeclarationStr=' '.join([p for p in funcDeclarationStr.split(' ') if p !='' ])#remove double spaces
        self.functionDeclaration=funcDeclarationStr;
        [templateStr,returnTypeStr,functionNameStr,parameterStr]=re.sub(r'(template\s?<[^>]*>|inline)\s*(void)\s([^(^\s]*)\((.*)\)\s*;',r'\1@\2@\3@\4',str).split('@')
        templateStr=re.sub('\s','',templateStr)
        functionNameStr=re.sub('\s','',functionNameStr)
        self.fullFunctionName=functionNameStr
        self.namespaces=functionNameStr.split('::')
        self.simpleFunctionName=self.namespaces[-1]
