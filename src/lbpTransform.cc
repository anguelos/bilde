    ...
    try{
        ...
        for(auto iterFname=inputFileNames.begin();iterFname != inputFileNames.end();++iterFname){
            ...
            for(auto radIter=radii.begin();radIter!=radii.end();++radIter){
                ...
                std::string ofName=(*iterFname)+".lbp_r"+std::to_string(int(*radIter))+"_t"+cmpThreshold+"_S"+interpolation+"_lbp.png";
                cv::imwrite(ofName,outImg,{CV_IMWRITE_PNG_COMPRESSION,0});
            }
            out<<"\n";
